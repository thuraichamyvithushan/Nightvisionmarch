                ...
                <div className="max-w-7xl mx-auto flex justify-between h-16 items-center">
                    <Link to="/dashboard" className="flex items-center group" onClick={closeMenu}>
                        <div className="transform group-hover:scale-105 transition-transform duration-300">
                            <img src="/assets/logo.png" alt="Night Vision" className="h-11 w-auto" />
                        </div>
                    </Link>
                    ...
                    <div className="hidden md:flex items-center space-x-4">
                        <div className="relative">
                            ...
                            >
                                <div className="h-8 w-8 rounded-xl bg-gradient-to-br from-sky-500 to-sky-700 flex items-center justify-center text-white font-bold text-sm shadow-md">
                            ...
                            {showProfileMenu && (
                                <div className="absolute right-0 mt-3 w-64 bg-white rounded-2xl shadow-2xl border border-gray-100 py-3 animate-fade-in overflow-hidden">
                                    <div className="px-5 py-3 border-b border-gray-50 mb-2">
                                        ...
                                        <div className="mt-2 inline-flex items-center px-2.5 py-0.5 rounded-full bg-sky-100 text-sky-700 text-[10px] font-bold uppercase tracking-wider">
                                            {role}
                                        </div>
                                    </div>
                                    <Link
                                    ...
                                    <button
                                        ...
                                        className="w-full text-left px-5 py-3 text-sm font-bold text-sky-600 hover:bg-sky-50 transition-colors duration-300 flex items-center space-x-3"
                                    >
                    ...
                    <div className="md:hidden flex items-center space-x-2">
                        <button
                            onClick={handleLogout}
                            className="p-2 rounded-xl bg-sky-50 text-sky-600 hover:bg-sky-600 hover:text-white transition-all duration-300 shadow-sm"
                            title="Log out"
                        >
                        ...
                        <button
                            onClick={toggleMenu}
                            className="p-2 rounded-xl bg-gray-100 text-gray-600 hover:bg-sky-50 hover:text-sky-600 transition-all duration-300"
                        >
                ...
                <div className={`md:hidden overflow-hidden transition-all duration-500 ease-in-out ${isMenuOpen ? 'max-h-[500px] opacity-100 py-4' : 'max-h-0 opacity-0'}`}>
                    <div className="space-y-2 px-2">
                        ...
                        <div className="pt-4 mt-4 border-t border-gray-100">
                            <div className="px-4 py-3 bg-sky-50 rounded-2xl flex items-center justify-between mb-4">
                                <div>
                                    <p className="text-[10px] font-bold text-sky-400 uppercase tracking-widest leading-tight">Signed in as</p>
                                    <p className="text-sm font-bold text-sky-800 truncate">{user?.email}</p>
                                </div>
                                <div className="h-10 w-10 rounded-xl bg-sky-600 flex items-center justify-center text-white font-bold shadow-md">
                                    {user?.email?.charAt(0).toUpperCase() || 'A'}
                                </div>
                            </div>
                            <Link
                            ...
                            <button
                                onClick={handleLogout}
                                className="w-full py-4 text-center text-base font-bold text-sky-600 bg-sky-50 hover:bg-sky-100 rounded-2xl transition-all duration-300"
                            >
                                Log out
                            </button>
                        </div>
